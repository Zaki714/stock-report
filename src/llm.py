...
# ── 國際題材歸納 ───────────────────────────────────────
GLOBAL_THEME_SYSTEM = """你是一位追蹤全球資金流向的總體策略分析師。

輸入：一批國際財經頭條（Bloomberg/Reuters/WSJ 等）、以及美股類股 ETF 的價格動能與量能倍數。

任務：歸納出本週國際市場最受關注的 1-3 個題材敘事，並評估其可信度。

評估準則：
- 高頻關鍵字：同一敘事在多家外電、多個查詢字串重複出現 → 關注度高
- 資金流佐證：對應類股 ETF 量增價漲 → 資金流入，支持題材；量增價跌 → 資金流出
- 機構態度：投行評等調升／調降家數，一致性越高，信心度越高；分歧則降一級
- 只有頭條熱度但無資金流或機構佐證 → verdict 給 watch，不要給 real

信心度：high / mid / low
真假判定：real / watch

只輸出 JSON，不要任何前後說明或 markdown 標記：
{
  "themes": [
    {"name": "題材名稱", "summary": "一句話說明", "confidence": "high", "verdict": "real",
     "reasoning": "判斷理由", "keywords": ["關鍵字1", "關鍵字2"],
     "etf_signal": "對應 ETF 的資金流向描述",
     "institution_signal": "機構評等一致性描述",
     "stocks": [{"code": "NVDA", "name": "Nvidia"}]}
  ]
}"""


def global_theme_digest(headlines: list[dict], etf_flows: list[dict]) -> dict:
    """把國際頭條 + ETF 資金流向歸納成題材。回傳 {"themes": [...]}。"""
    if DRY_RUN:
        return mock.llm_global_theme_response()

    head_lines = "\n".join(
        f"- [{h.get('source', '?')}] {h.get('title', '')}" for h in headlines
    )
    etf_lines = "\n".join(
        f"- {e.get('name', '')}：{e.get('change_pct', 0):+.2f}%　"
        f"量能 {e.get('volume_ratio', 'N/A')} 倍　{e.get('flow', '')}"
        for e in etf_flows
    )
    user = (f"國際財經頭條：\n{head_lines}\n\n類股 ETF 資金流向：\n{etf_lines}")

    try:
        result = _parse_json(_call(GLOBAL_THEME_SYSTEM, user))
        return result if isinstance(result, dict) and "themes" in result else {"themes": []}
    except Exception as exc:
        print(f"[llm] 國際題材歸納失敗：{exc}")
        return {"themes": []}
