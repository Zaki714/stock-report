...
def mark_safe(html: str) -> Markup:
    """把自家產生的 SVG／HTML 片段標記為安全，避免被 autoescape 轉義。

    只用於本專案自己組出來的字串（例如 charts.py 的 SVG），不要拿去包外部輸入。
    """
...
def decorate_theme(theme: dict) -> dict:
    ...
    theme["scope_label"] = SCOPE_LABEL.get(theme.get("scope", "tw"), "台股")
...
def render_monthly(context: dict, out_name: str) -> Path:
    """月報頁面：事後驗證成績單、本月題材增減、信心度分布。"""
    cfg = load_config()
    tpl = _env().get_template("monthly.html")
...
def render_holiday_wrapup(context: dict, out_name: str) -> Path:
    """長假彙整報告：休市期間國際盤逐日變化，避免開紅盤被跳空嚇到。"""
    cfg = load_config()
    tpl = _env().get_template("holiday_wrapup.html")
...
def _collect_reports() -> list[dict]:
    ...
    for f in sorted(reports_dir.glob("*.html"), reverse=True):
        ...
        kind = {"morning": "早報", "evening": "盤後", "holiday": "假日功課",
                "weekly": "週報", "monthly": "月報", "wrapup": "長假彙整"}.get(kind_key, "報告")
        entries.append({
