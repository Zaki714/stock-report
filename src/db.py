...
def themes_first_seen_between(start: str, end: str) -> list[dict]:
    """指定期間內首次建檔的題材 —— 月報「本月新增題材」用。"""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM themes WHERE first_seen BETWEEN ? AND ? ORDER BY first_seen",
            (start, end),
        ).fetchall()
        return [dict(r) for r in rows]


def themes_retired_between(start: str, end: str) -> list[dict]:
    """指定期間內最後一次有訊號、且目前已休眠／退場的題材 —— 月報「本月退場」用。"""
    with get_conn() as conn:
        rows = conn.execute(
            """SELECT * FROM themes
               WHERE status IN ('dormant', 'archived')
                 AND last_signal_date BETWEEN ? AND ?
...
def confidence_distribution() -> list[dict]:
    """追蹤中題材依信心度分組計數 —— 月報信心度分布用。"""
    with get_conn() as conn:
        rows = conn.execute(
            """SELECT confidence, COUNT(*) AS n FROM themes
               WHERE status='active' GROUP BY confidence
               ORDER BY CASE confidence WHEN 'high' THEN 0 WHEN 'mid' THEN 1 ELSE 2 END"""
        ).fetchall()
        return [dict(r) for r in rows]


def theme_confidence_series(theme_id: int) -> list[dict]:
    """單一題材的信心度時間序列 —— 折線圖用。"""
    rank = {"low": 1, "mid": 2, "high": 3}
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT date, confidence FROM theme_updates WHERE theme_id=? ORDER BY date",
            (theme_id,),
        ).fetchall()
        return [{"date": r["date"], "level": rank.get(r["confidence"], 2)} for r in rows]
