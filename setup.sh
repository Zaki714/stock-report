...
# ── 1. Python 版本 ──────────────────────────────────
echo "[1/5] 檢查 Python..."
command -v python3 >/dev/null 2>&1 || fail "找不到 python3，請先安裝 Python 3.10 以上：https://www.python.org/downloads/"
...
if pip install --quiet --upgrade pip 2>/dev/null && \
   pip install --quiet -r requirements.txt 2>/dev/null; then
    ...
    if python3 -c "import requests, pandas, jinja2, yaml" 2>/dev/null; then
