...
| 時間 | 產出 |
|---|---|
| 每日 07:00 | 國際盤摘要（美股四大指數、總經、台積電 ADR、當日法說會預告） |
...
寫進同一個題材知識庫，`scope='intl'`）。當週第一個交易日的早報會附上**上週回顧**。
...
src/
  ...
  main.py                入口與分支
  fetchers/
    twse.py              台股（TWSE 開放 API）
    international.py     國際盤 + 類股 ETF 資金流向（Stooq）
    mops.py              法說會（公開資訊觀測站）
    news.py              國際財經頭條（Google News RSS，免 key）
    mock.py              DRY_RUN 假資料
  analysis/
