...
| 國際題材追蹤模組 | ✅ 完成 | `DRY_RUN=1 python -m src.main morning`，產出 2 個國際題材寫進知識庫（scope='intl'） |
| 週報（併入週一早報） | ✅ 完成 | 模擬連續多日後，當週第一個交易日早報帶出上週回顧與走勢圖 |
| 月報頁面 | ✅ 完成 | `DRY_RUN=1 python -m src.main monthly`，產出 `docs/reports/<date>-monthly.html` |
| 長假彙整報告 | ✅ 完成 | `DRY_RUN=1 python -m src.main wrapup`，休市期間國際盤逐日回顧 + 折線圖 |
| 視覺化（SVG 折線／分向長條） | ✅ 完成 | 純 SVG，無圖表庫；題材信心度 sparkline、外資買賣超趨勢、指數走勢 |

**仍待實測**：真實 API 欄位對應（TWSE / Stooq / MOPS）、GitHub Actions 排程、Telegram 推播
—— 這三項本質上需要真實憑證與雲端環境才能驗證，程式邏輯與容錯已就緒。
...
4. **國際題材追蹤模組** ✅ 已完成
   - `src/fetchers/news.py`：Google News RSS 抓外電頭條（免 key），標題去重
   - `src/fetchers/international.py::fetch_sector_etfs()`：類股 ETF 價格動能 + 量能倍數當資金流向近似值
   - `src/llm.py::global_theme_digest()`：LLM 統計高頻關鍵字、評估資金流與機構一致性
   - `src/analysis/global_themes.py`：串起來，寫進同一個知識庫（`scope='intl'`），早報一併呈現
   - `config.yaml` 的 `global_themes` 區塊可調查詢字串、ETF 清單、頭條上限
   - mock：`mock.global_headlines()` / `mock.sector_etf_flows()` / `mock.llm_global_theme_response()`

5. **週報** ✅ 已完成
   `src/analysis/recap.py::weekly_recap()` 從 `market_snapshots` 彙整上週表現，
   由 `run_morning()` 在 `market_calendar.is_first_trading_day_of_week()` 為真時併入早報，
   不獨立開排程。附加權指數走勢折線圖。

6. **月報頁面** ✅ 已完成
   `templates/monthly.html` + `render.render_monthly()`。
   `run_monthly()` 產出 `docs/reports/<date>-monthly.html`：事後驗證成績單、
   本月新增／退場題材（`db.themes_first_seen_between` / `themes_retired_between`）、信心度分布。

### 優先度低

7. **農曆年長假彙整報告** ✅ 已完成
   `run_holiday_wrapup()` + `templates/holiday_wrapup.html`。
   `is_last_day_before_reopen()` 為真時改呼叫這個（原本呼叫 `run_holiday()`）。
   `international.fetch_index_history()` 抓休市期間四大指數逐日變化，逐指數畫折線圖。

8. **視覺化** ✅ 已完成
   `src/analysis/charts.py`：純 SVG，無圖表庫。三種圖：
   - `line_chart`：指數走勢、週回顧
   - `diverging_bars`：外資單日買賣超趨勢（紅正綠負）
   - `sparkline`：題材信心度變化，塞在題材卡片角落
   全部吃 `base.html` 的 CSS 變數，深色主題與紅漲綠跌自動套用。

### 仍待使用者實測（需真實憑證／雲端）

- 真實 API 欄位對應：接上 TWSE / Stooq / MOPS 後對照實際回傳調整（程式已做中英雙重備援）
- Stooq 若擋爬蟲：改 `yfinance`（`pip install yfinance`），沿用現有 dict 格式
- GitHub Actions 排程與 Telegram 推播：推上 repo、設好 Secrets 後手動觸發一次驗證
